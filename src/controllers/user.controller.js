...
const changeCurrentPassword = asyncHandler(async (req,res) => {
    ...
    .status(200)
    .json(
        new ApiResponse(200 , await User.findById(user._id).select("-password") , "Password Changes Successfully")
    )
})
...
const getCurrentUser = asyncHandler(async (req,res) => {
    ...
    .status(200)
    .json(
        new ApiResponse(200 , user , "Use founded" )
    )
})
...
const getWatchHistory = asyncHandler(async (req , res) => {
    ...
    .json(
        new ApiResponse(
            200 , user[0].watchHisory , "watchHistory got fetched successfully"
        )
    )
})
